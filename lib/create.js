...
async function createProject (projectName) {
  try {
    ...
    if (exists) {
      // 项目重名时提醒用户
      console.log(symbols.error, chalk.red(`The project <${projectName}> already exists.`))
    } else {
      // 执行控制台交互
      inquirer
        .prompt([
        ...
        .then(async (answers) => {
          ...
          // 不存在时，就先等待下载模板，完成再执行下面的语句
          !exists && await dlTemplate()

          // Spinner 初始设置
          const createSpinner = ora(chalk.cyan('Creating project...'))
          // 开始执行等待动画
          createSpinner.start()
          ...
          for (var i = 0; i < multiFiles.length; i++) {
            // 这里记得使用 try ... catch ...，以便出错时可以终止掉 Spinner
            try {
              // 等待读取文件
              ...
              createSpinner.text = chalk.red(`create project failed. ${err}`)
              // 终止等待动画并显示 X 标志
              createSpinner.fail()
              // 退出进程
              process.exit()
          ...
          createSpinner.text = 'create project successful.'
          // 终止等待动画并显示 ✔ 标志
